...
for inst_resource in resources:
  ...
  if resource_type == 'AWS::EFS::FileSystem':
    filesystem_id = inst_resource['PhysicalResourceId'].split('/')[-1].split(':')[0]

  if resource_type == 'AWS::EFS::MountTarget':
    mounttarget_id = inst_resource['PhysicalResourceId'].split('/')[-1].split(':')[0]



print(job_def_id_1)
print(filesystem_id)
print(mounttarget_id)
