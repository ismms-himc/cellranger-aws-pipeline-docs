...
* [Processing-Run CSVs]: the computational team's pre-processing pipelines *(can these be auto-generated from the previous step? If so, why are they necessary?)*
* [Cell Ranger Required CSVs]: CSVs required by Cell Ranger software
...
- `Chemistry`: the name of the 10x kit chemistry being used (e.g. fiveprime)
...
- `Project`: the project the sample is a part of (*What is a project in this context? Do you mean study?*)
...
- `FASTQs`: name of the Seq-Run-FASTQ Set that is the result of a single sequencing run.
  - Name includes: `Loading Sample`, `10x Lane ID`, `BCL Run ID`, `Library Type` (our examples leave off redundant information in the FASTQ names)
  - Tracking the `BCL` name allows us to handle the common scenario where the same pooled library (e.g. tube of liquid) is 
- `Library`: 
- `Loading Sample`: 
- `10x Lane`: lane number (*lane index, they usually refer to lanes as letters*) a sample is loaded into, necessary for keeping track of a single sample being loaded into multiple lanes
...
- `To Output`: name of the output a FASTQ is contributing to (e.g. `FBM1`, `TCR1`) (*This concept of output is not clear to me, please clarify*)
...
|---|---|---|---|---|
| LF1 | HTO-1_H-101_3p_Lot# | threeprime | H-101 | ACTG |
| LF1 | HTO-2_H-102_3p_Lot# | threeprime | H-102 | ACTG |  
| LF1 | HTO-3_H-103_3p_Lot# | threeprime | H-103 | ACTG |  
| LF1 | HTO-4_H-104_3p_Lot# | threeprime | H-104 | ACTG |  
| LF1 |   CD3_A-101_3p_Lot# | threeprime | A-101 | ACTG |  
| LF1 |   CD4_A-102_3p_Lot# | threeprime | A-102 | ACTG |  
| LF1 |   CD8_A-103_3p_Lot# | threeprime | A-103 | ACTG |  

(*Are all the Oligo Sequences the same for all features in a given library? If not, please make it explicit using different sequences*)
...
- `HIMC Feature Name`: name of a feature (ADT or HTO) being measured
  ...
  - chemistry
  - the lot number (e.g. lot-1) (*What's the lot number?*). 
- `Chemistry`: the type of chemistry this feature is made for (*In order to simplify further processing, please adhere to [Cellranger chemistry standards](https://support.10xgenomics.com/single-cell-gene-expression/software/pipelines/latest/using/count)*)
...
- `To Output`: name of the output a FASTQ is contributing to (e.g. `FBM1`, `TCR1`) (*Again, the concept of **output** here is not clear to me*)
- `Seq-Lanes`: the number of sequencing lanes used    
- `Index Name`: the name of the `Sample Index` obtained from the [FASTQ-Level Spreadsheet]
- `Index Oligo`: the actual index oligo sequence obtained from a Sample Index Spreadsheet (**not documented yet**). The value will be `-` for GEX index oligos since Cell Ranger mkfastq knows the meaning the index names (e.g. `SI-GA-3`). (*Why is that needed? We have the dictionary ADT/HTO ID to sequence*)
...
- `Library Features`: this links a sample to its list of features in the [Library Features Spreadsheet]. The value is `-` if we are not measuring any ADTs or HTOs
- `Sample Name`: the name of the biological sample being processed (see [Glossary])
- `Project`: the project the sample is a part of (*What is a project? We work at the level of studies, the concept of project does not appear in the glossary. Please adhere to the common terminology used*)
- `HTO`: the name of the hash tag oligo (HTO) that is used to label this sample, the value will be `-` for a non-hashed sample
...
|---|---|---|---|---|---|---|
| LF1 | HTO-1_H-101_3p_Lot# | HTO-1 | R2  | seq-pattern  | AACAAGACCCTTGAG  | Antibody Capture  |  
| LF1 | HTO-2_H-101_3p_Lot# | HTO-2 | R2  | seq-pattern  | CCCTTGAGAACAAGA  | Antibody Capture  |  
| LF1 | HTO-3_H-101_3p_Lot# | HTO-3 | R2  | seq-pattern  | AACATTGAGACCCAG  | Antibody Capture  |  
| LF1 | HTO-4_H-101_3p_Lot# | HTO-4 | R2  | seq-pattern  | TGAAACAAGACCCTG  | Antibody Capture  |  
| LF1 |   CD3_A-101_3p_Lot# |   CD3 | R2  | seq-pattern  | AACAACTTGAGGACC  | Antibody Capture  |  
| LF1 |   CD4_A-102_3p_Lot# |   CD4 | R2  | seq-pattern  | GGACCAAACAACTTG  | Antibody Capture  |  
| LF1 |   CD8_A-103_3p_Lot# |   CD8 | R2  | seq-pattern  | GGACCACTTGAACAA  | Antibody Capture  |  
...
- `feature_type`: the type (e.g. Custom) from the list of acceptable feature types: `Antibody Capture` (preferred (*Since what we are indicating here are Antibodies, I think it is better to use Antibody Capture as a default here*)), `Custom` or `CRISPR Guide Capture` (see [docs](https://support.10xgenomics.com/single-cell-gene-expression/software/pipelines/latest/using/feature-bc-analysis))
...
| /path/to/fastqs/ | FQ1-ADT | Antibody Capture |
| /path/to/fastqs/ | FQ1-HTO | Antibody Capture |
...
# Output CSVs
A single processing run will produce the following spreadsheets: 1) Job Status CSV, 2) FASTQ meta-data, and 3) Outputs Meta-Data CSV
...
- `Status`: the current status of the job 
  - can be: `Pending`, `In-Progress`, `Finished`, or `Failed`. (*Please simplify to `Pending` and `Finished`; if a job fails, it can fail for a plethora of reasons, and in practical terms it may be hard to capture the failure (especially from Cellranger). Likewise, how can a job outputs itself as `In progress`? We are talking here about the output status after running a job*)
...
- `Expiration date for the link`: the expiration date for the link (*take into account that pre-signed URLs expire)
...
- `Output Path`: path on the S3 bucket to the outputs (*Do PIs need to see this? I think not; I think that this route should not be here*)
...
Three samples are run in two 10x chip lanes each (to double the number of measured cells) producing six GEX libraries. A pooled library is generated and sequenced twice (to get more reads per cell). The BCL files are de-multiplexed into twelve sets of Seq-Run FASTQ Sets and six corresponding FBM FASTQ Sets. Six runs of Cell Ranger `count` are run on the six FBM FASTQ Sets. 
...
Three ADT samples (*The term ADT sample is confusing here: do you mean `Loading Sample` that produces an ADT `Sequencing Library`?*) are run in three lanes producing three GEX and three ADT libraries. These libraries are pooled and sequenced twice. The BCL files are de-multiplexed into twelve Seq-Run FASTQ Sets and three FBM FASTQ Sets (each containing GEX and ADT FASTQs). Cell Ranger `count` (using feature barcoding) is run three times to produce three FBMs containing both GEX and ADT data. 

(*As a side note, I do not remember seeing scenarios 9 and 10 in practice. Shall we take them into consideration?*)
...
* **Cell Ranger Output**: Feature barcode matrix (FBM) which can be the product of several sequencing runs and BCL files, TCR/BCR contigs (*Calling `Cell Ranger Output` to simply the FBMs generated by Cell Ranger is oversimplifying. Cell Ranger also generates BAM files, reports as `web_summary.html` and `metrics_summary.csv` that are useful for customers and HIMC staff. If you are referring only to FBMs, please create a separate term in the glossary*)
