...
- `FASTQs`: name of the set of FASTQs generated by Cell Ranger Count - this is composed of the loading sample name appended with the `BCL Run ID`. This allows us to handle the common scenario where the same sequencing library (e.g. tube of liquid that can be aliquoted from) is sequenced more than once.
- `Loading Sample`: name of the sample that is loaded into a 10x chip lane (e.g. can consist of several samples via hashing).
- `Hashed Sample`: True/False value indicating whether hashing has been done (will be redundant with the `Loading Sample` naming convention).
- `10x Lane ID`: indicates the lane number a sample is loaded into
...
The `Index` column contains the oligo index IDs that can be looked up in [the `Features Table`](#TODO).
...
The 10x techs will produce the feature reference CSV (presumably via lookup tables).
